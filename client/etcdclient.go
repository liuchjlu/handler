...
func NewEtcdClient(etcdpath string) (*Etcd, error) {
	...
	etcd := new(Etcd)
	if err != nil {
		log.Errorf("client.NewEtcdClient:%+v", err)
		return etcd, err
	}
	kapi := client.NewKeysAPI(c)
	etcd.client = kapi
	log.Infof("ETCD Client:%+v", etcd)
	return etcd, err
}
...
func (e *Etcd) DeleteDir(dir string) error {
	_, err := e.client.Delete(context.Background(), dir, &client.DeleteOptions{Recursive: true, Dir: true})
	if err != nil {
		log.Errorln("etcdclient.DeleteDir():Get a error", err)
		return err
	}
	return nil
}
